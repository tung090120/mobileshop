...
if (isset($_GET['cat_id'])) {
    $cat_id = mysqli_real_escape_string($conn,(string)(int)$_GET['cat_id']);
    ...
    $sql_product = "SELECT * FROM product WHERE cat_id = '%s' LIMIT $per_row, $row_per_page";
    $query_product = mysqli_query($conn, sprintf($sql_product,$cat_id));

    // fetch cat_name
    $sql = "SELECT * FROM category WHERE cat_id = %s";
    $category = mysqli_fetch_array(mysqli_query($conn, sprintf($sql,$cat_id)));
}
...
<?php
// Thanh phân trang
$sql = "SELECT * FROM product WHERE cat_id = '%s'";
$query = mysqli_query($conn, sprintf($sql,$cat_id));
